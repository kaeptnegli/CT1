...
#define DMA_STREAM_ENABLE       0x00000001u
...
{
    /* Setup channel */
    dma->STREAM[stream].CR &= ~(0x7u << 25u);
    dma->STREAM[stream].CR |= ((uint32_t) init.channel << 25u);

    /* Setup direction */
    dma->STREAM[stream].CR &= ~(0x3u << 6u);
    dma->STREAM[stream].CR |= ((uint32_t) init.direction << 6u);

    /* Setup source and destination */
    dma->STREAM[stream].M1AR = 0u;

    assert(init.direction == HAL_DMA_PER_TO_MEM || init.direction == HAL_DMA_MEM_TO_PER
            || init.direction == HAL_DMA_MEM_TO_MEM);
    ...
    /* Setup buffer size */
    dma->STREAM[stream].CR &= ~(0xfu << 11u);
    dma->STREAM[stream].CR |= ((uint32_t) init.size << 11u);
    dma->STREAM[stream].CR |= ((uint32_t) init.size << 13u);

    /* Setup nr of transfers */
    dma->STREAM[stream].NDTR = init.nr_transactions;

    /* Setup circular mode */
    dma->STREAM[stream].CR &= ~(0x1u << 8u);
    dma->STREAM[stream].CR |= ((uint32_t) init.continous << 8u);
}
