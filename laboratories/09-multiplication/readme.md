...
In this laboratory you learn, how binary multiplication is implemented.
...
product of two digit numbers is therefore `99 * 99 = 9801`. We can
...
`3 * 12`. It means that we add twelve three times. This is already a
method to implement multiplication.
...
Implement a function to multiply to unsigned 16 bit operands. The
operands are passed in register `r0` and `r1`. The result shall be
return in register `r0`.
...
4)  Now implement the multiplication yourself. Test your implementation
    and ensure it passes the test.
...
code in the function `operation` in `mul_u32.s`. Test your code and
