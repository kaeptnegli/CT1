...
wide value is read from the DIP-switches `SW[15..8]` (operand A) and
another from `SW[7..0]` (operand B). Both operands shall be expanded to
32 bit, as mentioned in the introduction. Display the most significant
byte of the sum on `LED[7..0]` and the most significant byte of the
difference on `LED[23..16]`.
...
the LEDs. The flags from the addition shall be displayed on
`LED[15..12]` and the ones from the subtraction on `LED[31..28]` (See
the figure below).
