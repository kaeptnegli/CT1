...
`./common/pdf.mk` is a make include file with rules to convert
Markdown to PDF.

`./common/root.mk` is a make include file with rules to build and link
binaries from C and assembly sources.

`./common/template.tex` is a LaTeX template for pandoc to customise
the PDF output of pandoc.

`./common/trailing-whitespace.sh` is a bash script to detect and
remove trailing whitespace and optionally convert line endings to
unix style (lf).
